...
function uniqueLibraryFolderNames(dependencies: LibraryDependency[] = [], toFolderName: (dependency: LibraryDependency) => string): string[] {
    return Array.from(new Set(dependencies.map(dependency => toFolderName(dependency))));
}
...
export class H5PStandalone {
    ...
    async prepareH5PEnvironment(contentId, options: Options): Promise<H5PIntegration> {
        ...
        H5PIntegration.theme = mergeObject({density: 'large'}, H5PIntegration.theme || {}, options.theme || {});
    ...
    async findAllDependencies(h5pJsonContent: H5PPackageDefinition,
        ...
        return this.loadDependencies(Array.from(new Set(directDependencyFolderNames)), [], librariesPath, assetsRequestFetchOptions);
    }
    ...
    async findLibraryDependencies(libraryFolderName: string,
        ...
        const dependencies = uniqueLibraryFolderNames([
            ...(library.preloadedDependencies || []),
            ...(library.dynamicDependencies || [])
        ], this.libraryToFolderName.bind(this));
