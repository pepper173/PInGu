...
## Updating vendored H5P binary assets

Some upstream H5P core updates also change vendored binary assets such as fonts and images. Because binary file updates are not always convenient to review in code review tooling, this repository keeps those updates out of normal source changes and provides a helper script instead:

```bash
yarn sync:h5p:binaries
```

The script downloads the latest `h5p/h5p-php-library` archive from GitHub and syncs `vendor/h5p/fonts` and `vendor/h5p/images` locally so you can review and commit those binary changes separately when needed.
